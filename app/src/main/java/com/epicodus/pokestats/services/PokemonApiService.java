...
public class PokemonApiService {
    ...
    public static User processUser(Response response){
        ...
        try {
            ...
            if (response.isSuccessful()) {
                ...
                if(userJSON.optBoolean("error", false)){
                    ...
                    int max_pokemon = userJSON.optInt("max_pokemon", 0);
                    ...
                    int next_level_xp = userJSON.optInt("next_level_xp", 0);
                    ...
                    user = new User(team, username, pokecoin, stardust, max_item, creation_time, max_pokemon, revive_count, razz_count,
                            ...
                            greatball_count, potion_count, sup_potion_count, hyp_potion_count, egg_count, pokemon_count, pokemons_captured, ultraball_count, masterball_count);
    ...
    public static ArrayList<Pokemon> processPokemon(Response response){
        ...
        try {
            String jsonData = response.body().string();
            if (response.isSuccessful()) {
                JSONArray pokemonArray = new JSONArray(jsonData);
                Gson gson = new Gson();
                for(int i = 0; i< pokemonArray.length(); i++){
                    JSONObject pokemonJSON = pokemonArray.getJSONObject(i);
                    Pokemon pokemon = gson.fromJson(pokemonJSON.toString(), Pokemon.class);
